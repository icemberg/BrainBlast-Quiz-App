...
public class OAuth2LoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (existingUser.isPresent()) {
            ...
            user.setRole("USER"); 
            // Password is not applicable for OAuth2 users, but we might set a dummy one or handle it in specific way. 
            // Since we use BCrypt, we can set a random unguessable password.
            user.setPassword(org.springframework.security.crypto.bcrypt.BCrypt.hashpw(java.util.UUID.randomUUID().toString(), org.springframework.security.crypto.bcrypt.BCrypt.gensalt()));
            userDao.save(user);
        }
        ...
        String targetUrl = "http://localhost:5173/oauth2/redirect?token=" + token + "&role=" + user.getRole() + "&username=" + user.getUsername();
